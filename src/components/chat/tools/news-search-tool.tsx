...
interface NewsSearchToolProps {
    toolCallId: string;
    input?: {
        news_search_queries?: string[];
    };
    output?: {
...
export function NewsSearchTool({ toolCallId, input, output }: NewsSearchToolProps) {
    if (output) {
        return (
    ...
    return (
        <Accordion className="w-full">
            <AccordionItem value={toolCallId} className="border bg-neutral-900 rounded-md px-4">
                ...
                <AccordionContent className="space-y-1">
                    {input?.news_search_queries?.map((query: string, index: number) => (
                        <Disclosure
                            key={`${toolCallId}-${index}`}
                            className="w-full flex flex-col gap-2"
                        >
